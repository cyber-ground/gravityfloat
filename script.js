...
class Ball {
  ...
  eventHandler() {  // gravity switch to 0G
    canvas.addEventListener('click', (e) => {
      ...
      if(e.clientY < 100 && 
        e.clientX < canvas.width - 100 && e.clientX > canvas.width / 2) { 
        console.log('on');
        if(canvas.classList.contains('blank_color')) return;
          canvas.classList.add('blank_color');
          randomColor = colors[Math.floor(Math.random() * colors.length)];
      }
      if(e.clientY < 100 &&  // top //
        (e.clientX > 100 && e.clientX < canvas.width / 2)) {
          console.log('off');
        canvas.classList.remove('blank_color');
      }
    });
  }
  draw() {
  ...
  mouseColDetect() {
    ...
    if(distance - this.radius < 0) {
      ...
      if(canvas.classList.contains('blank_color')) {
